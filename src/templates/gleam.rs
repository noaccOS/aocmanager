...
const TEMPLATE_DIR: include_dir::Dir<'static> = include_dir!("./templates/gleam");

#[derive(Debug, Default, Clone, serde::Deserialize, serde::Serialize)]
pub struct Gleam {}

impl AoCTemplate for Gleam {
    ...
    fn run_samples(&self, variant: super::variant::RunVariant) -> Command {
        match variant {
            RunVariant::A => {
                let mut command = Command::new("gleam");
                command.args(["run", "samples-a"]);
                command
            }
            RunVariant::B => {
                let mut command = Command::new("gleam");
                command.args(["run", "samples-b"]);
                command
            }
            RunVariant::Both => {
                let mut command = Command::new("gleam");
                command.args(["run", "samples"]);
                command
            }
        }
    }
